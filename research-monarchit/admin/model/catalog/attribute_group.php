...
class ModelCatalogAttributeGroup extends Model {
	...
	public function getAttributeGroups($data = array()) {
        ...
        //Admin attribute_group filter	
		$filter =preg_replace('/[^A-Za-z0-9 \-]/', '', $this->db->escape($data['filter_name']));
	...
	public function getTotalAttributeGroups() {
		...
		if (!empty($this->request->get['filter_name'])) {
				...
				$sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "attribute_group cp INNER JOIN " . DB_PREFIX . "attribute_group_description cd2 ON (cp.attribute_group_id = cd2.attribute_group_id) WHERE cd2.name LIKE '" . $this->db->escape($filter_name) . "%'";
			} else {
				$sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "attribute_group";
			}
			$query = $this->db->query($sql);
