...
class ControllerExtensionModulePcBuilderFilter extends Controller {
	public function index() {
		...
		$this->document->addScript('catalog/view/javascript/jquery/jquery-ui-1.13.0/jquery.ui.touch-punch.min.js');
		...
		// Get incompatible categories if there is any selected products
		$incompatible_categories = array();

		if (!empty($pc_builder_selected_products)) {
			foreach ($pc_builder_selected_products as $selected_product) {
				// Get selected product info
				$selected_product_info = $this->model_extension_pc_builder_pc_builder_product->getProduct($selected_product['product_id']);

				if ($selected_product_info) {
					$selected_product_categories = $this->model_extension_pc_builder_pc_builder_product->getProductCategories($selected_product_info['product_id']);

					if ($selected_product_categories) {
						foreach ($selected_product_categories as $selected_product_category_id) {
							$selected_product_category_incompatible_categories = $this->model_extension_pc_builder_pc_builder_product->getCategoryIncompatibleCategory($selected_product_category_id);
		...
		//print_r($incompatible_categories);

		if (isset($this->request->get['min_price'])) {
			$min_price = $this->request->get['min_price'];
		} else {
			$min_price = $this->config->get('module_pc_builder_filter_price_slider_min');
		}

		if (isset($this->request->get['max_price'])) {
			$max_price = $this->request->get['max_price'];
		} else {
			$max_price = $this->config->get('module_pc_builder_filter_price_slider_max');
		}
		...
		$data['slider_step'] = $this->config->get('module_pc_builder_filter_price_slider_step');
		...
		foreach ($category_ids as $category_id) {
			if (!in_array($category_id, $incompatible_categories)) {
