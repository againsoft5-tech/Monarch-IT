...
class ControllerExtensionPcBuilderPcBuilder extends Controller {
	public function index() {
		...
		$data['description'] = html_entity_decode($this->config->get('pc_builder_description'), ENT_QUOTES, 'UTF-8');
		
		$this->document->setTitle($this->config->get('pc_builder_meta_title'));
        $this->document->setDescription($this->config->get('pc_builder_meta_description'));
        $this->document->setKeywords($this->config->get('pc_builder_meta_keyword'));
		...
		foreach ($pc_builder_categories as $pc_builder_category) {
			...
			foreach ($pc_builder_components as $pc_builder_component) {
				if ($pc_builder_component['image']) {
					$thumb = $this->model_tool_image->resize($pc_builder_component['image'], 48, 48);
				} else {
					$thumb = $this->model_tool_image->resize('placeholder.png', 48, 48);
				}
		...
		if (!empty($this->request->get['build']) || !empty($this->session->data['pc_builder_selected_products'])) {
			$data['is_component_selected'] = true;
		} else {
			$data['is_component_selected'] = false;
		}
		...
		$data['text_sub_heading'] = sprintf($this->language->get('text_sub_heading'), $this->config->get('config_name'));
