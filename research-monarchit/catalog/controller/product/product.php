	...
	public function index() {
		...
		if ($product_info) {
			...
			if (!is_null($product_info['special']) && (float)$product_info['special'] >= 0) {
				...
				$product_special_price = $product_info['special'];
			} else {
				$data['special'] = false;
           ...
           if($product_special_price > 0){
             $emi_price = $product_special_price + ($product_special_price * 0.07);
            }else{
               $emi_price = $product_info['price'] + ($product_info['price'] * 0.07);
            }
            //EMI price 7% end
    
            $six_month_emi_price = ($emi_price / 6);
            $data['per_month_emi_price'] = number_format($six_month_emi_price);
            $data['emi_price'] = $emi_price;
            $data['emi_final_price'] = $this->currency->format($emi_price, $this->session->data['currency']);
