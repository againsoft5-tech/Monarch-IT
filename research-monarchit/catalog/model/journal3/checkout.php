...
class ModelJournal3Checkout extends Model {
	...
	private function payment_method() {
        ...
        // Check if the selected payment method exists in available payment methods
        if (!isset($this->session->data['payment_methods'][$code])) {
            $code = null;
        }
    
        // Check if there's an EMI product in the cart
        $emi_in_cart = false;
        foreach ($this->cart->getProducts() as $product) {
            if (isset($product['emi_option']) && $product['emi_option'] == 2) { // Assuming 2 represents EMI
                $emi_in_cart = true;
                break;
            }
        }
    
        // Ensure payment_methods is set before modifying it
        if (isset($this->session->data['payment_methods']) && is_array($this->session->data['payment_methods'])) {
            // Remove COD if EMI product is in the cart
            if ($emi_in_cart && isset($this->session->data['payment_methods']['cod'])) {
                unset($this->session->data['payment_methods']['cod']);
            }
        }
    
        // Auto-select payment method if enabled in Journal3 settings
        if ($code === null && $this->journal3->settings->get('sectionPaymentAutoSelect')) {
            foreach ($this->session->data['payment_methods'] as $payment_method) {
                if ($emi_in_cart && $payment_method['code'] == 'cod') {
                    continue; // Skip COD selection if EMI is in the cart
                }
	...
	private function payment_method1() {
		$code = Arr::get($this->session->data, 'payment_method.code');
